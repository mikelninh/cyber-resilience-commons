...
import {TEMPLATES,SCENARIOS,RECOMMENDED_BASELINE,CONTROL_CATALOG} from './data.js'
import {simulate,recommendations,recoveryPlan,customizeTemplate,buildBrief} from './simulator.js'
...
test('custom organization changes planning scale without mutating source template',()=>{
  const custom=customizeTemplate(city,{name:'Example District',identityCount:350,criticalService:'Permits portal',sensitiveData:'Resident records'})
  assert.equal(custom.label,'Example District')
  assert.ok(custom.scale<city.scale)
  assert.equal(city.label,'City / public administration')
})

test('download brief includes priorities, recovery and truth boundary',()=>{
  const brief=buildBrief({template:city,scenario:stolen,controls:city.defaults,catalog:CONTROL_CATALOG,scenarios:SCENARIOS})
  assert.match(brief,/Highest-leverage next actions/)
  assert.match(brief,/Recovery drill assumptions/)
  assert.match(brief,/synthetic planning model/i)
  assert.match(brief,/explicitly authorized/i)
})
