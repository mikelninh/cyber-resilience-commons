export const CONTROL_CATALOG = {
  mfa: {
    ...
    prove: 'Test an owned account with a stolen-password scenario and verify sensitive access still requires the second factor.'
  },
  leastPrivilege: {
    ...
    prove: 'Use a test identity and verify it cannot read, modify, or administer systems outside its documented role.'
  },
  segmentation: {
    ...
    prove: 'From an authorized test segment, verify routes and service identities cannot reach protected systems without an explicit path.'
  },
  egress: {
    ...
    prove: 'In a safe test environment, generate abnormal bulk-transfer behavior and verify it is detected, rate-limited or blocked.'
  },
  backups: {
    ...
    prove: 'Restore a representative critical service from protected backup and record the actual recovery time.'
  },
  vendorIsolation: {
    ...
    prove: 'Review and test a vendor identity against an explicit allow-list of systems, actions and data it is permitted to access.'
  },
  detection: {
    ...
    prove: 'Run an authorized abnormal-access drill and measure time from first signal to account or endpoint isolation.'
  },
  sessionRevocation: {
    ...
    prove: 'Issue a test session, rotate credentials, revoke the session centrally and verify the old token can no longer act.'
  }
}

export const TEMPLATES = [
  {
    id: 'city', label: 'City / public administration', shortLabel: 'City',
    description: 'Citizen services, document systems, staff identities and external providers.', scale: 1,
    assets: [
      ['Identity', '3,500 staff identities'], ['Critical service', 'Citizen service portal'],
      ['Internal service', 'Document & case management'], ['Sensitive data', 'Citizen and employee records'],
      ['External access', 'IT / software vendors'], ['Recovery', 'Central backup vault']
    ],
  ...
  {
    id: 'hospital', label: 'Hospital / care provider', shortLabel: 'Hospital',
    description: 'Clinical systems, staff accounts, connected vendors and time-critical services.', scale: .72,
    assets: [
      ['Identity', '1,200 clinical + admin accounts'], ['Critical service', 'Electronic health record'],
      ['Internal service', 'Scheduling & lab systems'], ['Sensitive data', 'Patient and workforce records'],
      ['External access', 'Medical / IT vendors'], ['Recovery', 'Clinical backup vault']
    ],
  ...
  {
    id: 'school', label: 'School / education network', shortLabel: 'School',
    description: 'Student records, learning tools, staff accounts and shared cloud services.', scale: .35,
    assets: [
      ['Identity', '450 staff + admin accounts'], ['Critical service', 'Student information system'],
      ['Internal service', 'Learning & collaboration suite'], ['Sensitive data', 'Student and family records'],
      ['External access', 'Education SaaS providers'], ['Recovery', 'Cloud + offline backups']
    ],
    defaults: {mfa:false, leastPrivilege:false, segmentation:false, egress:false, backups:true, vendorIsolation:false, detection:false, sessionRevocation:false}
  },
  {
    id: 'ngo', label: 'NGO / nonprofit', shortLabel: 'NGO',
    description: 'Donor CRM, communications, shared files, finance and third-party tools.', scale: .24,
    assets: [
      ['Identity', '120 staff + volunteer accounts'], ['Critical service', 'Donor / beneficiary CRM'],
      ['Internal service', 'Email & shared drive'], ['Sensitive data', 'Donor and beneficiary records'],
      ['External access', 'Fundraising / IT vendors'], ['Recovery', 'Cloud backup set']
    ],
  ...
  {
    id: 'business', label: 'Small business', shortLabel: 'Business',
    description: 'Email, CRM, accounting, storefront and cloud documents.', scale: .16,
    assets: [
      ['Identity', '40 staff accounts'], ['Critical service', 'Accounting / payments'],
      ['Internal service', 'CRM & email'], ['Sensitive data', 'Customer and finance records'],
      ['External access', 'Managed IT / SaaS'], ['Recovery', 'Cloud + local backup']
    ],
    defaults: {mfa:false, leastPrivilege:false, segmentation:false, egress:false, backups:true, vendorIsolation:false, detection:false, sessionRevocation:false}
  }
]
...
export const SCENARIOS = [
  {
    id: 'stolen-credentials', label: 'Stolen credentials',
  ...
  {
    id: 'ransomware', label: 'Ransomware on one device',
  ...
  {
    id: 'vendor-compromise', label: 'Compromised vendor',
  ...
  {
    id: 'bulk-exfiltration', label: 'Bulk data exfiltration',
  ...
  {
    id: 'oauth-token', label: 'Stolen SaaS / OAuth token',
...
export const RECOMMENDED_BASELINE = {
  mfa:true, leastPrivilege:true, segmentation:true, egress:true,
  backups:true, vendorIsolation:true, detection:true, sessionRevocation:true
}
